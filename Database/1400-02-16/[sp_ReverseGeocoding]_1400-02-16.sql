USE [OSM]
GO
/****** Object:  StoredProcedure [dbo].[sp_ReverseGeocoding]    Script Date: 5/6/2021 6:15:20 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	select @BufferXYStartNode =  geometry::STGeomFromText('POINT('+@XY+')',4326).STBuffer(0.01)
	...
	select top 1 @CityGeometry = place_a.Shape, @CityOsmId = place_a.osm_id, @CityName = place_a.name from OSM_PLACES_A place_a where place_a.fclass in('city', 'town', 'island') and @XYGeometry.STIntersects(place_a.Shape) = 1
	...
	select top 1 @PointOfInterrest = place.name from OSM_PLACES place  where CountyOsmId = @CountyOsmId and @BufferXYEndNode.STIntersects(place.Shape) = 1 and place.fclass in('locality', 'village', 'town') order by @XYGeometry.STDistance(place.SHAPE) asc
	...
	select top 1 @EndNode = A.osm_id from  (
		select Road.osm_id, Road.Shape, Road.fclass from OSM_ROADS Road where @CityGeometry.STIntersects(Road.Shape) = 1 
	) A where (A.fclass ='primary' or A.fclass ='trunk') and @BufferXYEndNode.STIntersects(A.SHAPE)=1 order by @XYGeometry.STDistance(A.SHAPE) asc 
