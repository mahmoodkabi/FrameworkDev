USE [OSM]
GO
/****** Object:  StoredProcedure [dbo].[sp_ReverseGeocoding]    Script Date: 5/14/2021 6:39:35 PM ******/
SET ANSI_NULLS ON
GO
...
--exec [dbo].[sp_ReverseGeocoding] @XY='51.8973268637075 32.4754657840897'
-- exec [dbo].[sp_ReverseGeocoding] @XY='51.8351993786578 32.7020686376982'
...
BEGIN
	...
	declare @Reorderaddress nvarchar(2000), @StreetName nvarchar(500)
	...
	if isnull(@CityName,'') != '' and isnull(@ZoneName,'') != ''
		...
		set @CountyCityZone = isnull(@CountyName,'') + ' - ' + N' شهر' + isnull(@CityName,'')
	...
	Select *  Into #IranNodeCounty from IranNode where CountyOsmId = @CountyOsmId

	--پيدا كردن پوينت آف اينترست
	select top 1 @PointOfInterrest = place.name from OSM_PLACES place  where CountyOsmId = @CountyOsmId and geometry::STGeomFromText('POINT('+@XY+')',4326).STBuffer(0.1) .STIntersects(place.Shape) = 1 and place.fclass in('village', 'town') order by @XYGeometry.STDistance(place.SHAPE) asc
    ...
    BEGIN
        ...
        FROM #Discovered d JOIN #IranNodeCounty e ON d.Id = e.FromNode
        WHERE e.ToNode NOT IN (SELECT Id From #Discovered) and e.fclass != 'footway'
        GROUP BY e.ToNode
    END;
   ...
   --select * from #Discovered;
	...
	set  @Reorderaddress = REPLACE(@Reorderaddress, '---', '-')
	set  @Reorderaddress = REPLACE(@Reorderaddress, '--', '-')
	set  @Reorderaddress = REPLACE(@Reorderaddress, N'شهرشهر', N'شهر')
